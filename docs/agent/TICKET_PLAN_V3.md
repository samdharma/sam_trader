...
| 0.7 | `sam-p0-futu-opend` | Futu OpenD service definition | task | port | `futuopen/futu-opend:latest`. Port 11111. Env vars: FUTU_LOGIN_ACCOUNT, FUTU_LOGIN_PWD_MD5, FUTU_TRADE_PASSWORD. Volume: `futu_opend_data`. Health check. Profile: `futu`. |
...
| # | Ticket ID | Title | Type | Port | AC Highlights |
|---|-----------|-------|------|------|---------------|
| 2.1 | `sam-p2-connection` | Port Futu connection manager from v2 | task | port | Port `connection.py` from v2. Re-verify monkey-patch for `is_async_connect`. Update module refs to `sam_trader`. Add `FUTU_LOGIN_ACCOUNT` env→context wiring. |
...
| 3.2 | `sam-p3-exec-client` | FutuLiveExecutionClient: order submit/modify/cancel, fill handling, account discovery | task | new | `adapters/futu/execution.py`. Subclass `LiveExecutionClient`. `_submit_order`, `_modify_order`, `_cancel_order` via `OpenSecTradeContext`. Bracket order support via order_factory. Order/fill push handling. `_register_venue_account_aliases` for multi-market accounts. Account auto-discovery via `get_acc_list`. Position reconciliation on connect. Adapt patterns from nautilus-futu execution.py. |
...
| # | Ticket ID | Title | Type | Port | AC Highlights |
|---|-----------|-------|------|------|---------------|
| 5.1 | `sam-p5-ib-port` | Port IBKR adapter from v2: config, factories, instrument provider | task | port | Port IBKR config section from v2 `main.py`. `InteractiveBrokersDataClientConfig`, `InteractiveBrokersExecClientConfig`. `InteractiveBrokersLiveDataClientFactory`. `InteractiveBrokersLiveExecClientFactory`. Wire into `build_trading_node()` under `ib_enabled` flag. |
...
| 9.3 | `sam-p9-risk` | Risk manager: Monte Carlo position sizer, pre-trade checks, portfolio heat monitor | task | new | Monte Carlo simulation for position sizing. Pre-trade risk checks (max exposure, daily loss, margin). Portfolio heat monitor. Integrity checker (unreasonable size, duplicate orders). |
...
| 9.5 | `sam-p9-orch` | Pipeline orchestrator: sequential executor, sanity check, bundle creator, readiness report | task | new | Run pipeline steps in sequence: scan → AI → risk → regime → bundles. Pre-market sanity check (no anomalies). Execution bundle YAML generator. Daily readiness report (candidates, risks, recommendations). Operator notification (console + optional webhook). |
