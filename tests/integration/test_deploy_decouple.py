...
class TestSamHotfix:
    @patch("sam_trader.services.cli._run")
    def test_hotfix_copies_module_without_restart(
        self, mock_run: Any, tmp_path: Path
    ) -> None:
        """sam hotfix <module> must copy file without triggering full restart."""
        src = tmp_path / "strategies" / "orb.py"
        src.parent.mkdir(parents=True)
        src.write_text("# updated strategy")

        rc = main(["hotfix", str(src)])
        assert rc == 0

        all_calls = [c[0][0] for c in mock_run.call_args_list]
        assert any("cp" in c for c in all_calls)
        assert any(str(src) in str(c) for c in all_calls)
        assert any("sam-trader" in str(c) for c in all_calls)

        # Must trigger hotfix watcher (touch trigger file)
        assert any("touch" in c for c in all_calls)
        assert any(".hotfix_trigger" in str(c) for c in all_calls)

        # Must NOT call docker compose restart (full restart)
        assert not any(
            "restart" in str(c).lower() and "compose" in str(c).lower()
            for c in all_calls
        )


@pytest.mark.integration
class TestSamRollback:
    @patch("sam_trader.services.cli._run")
    @patch("sam_trader.services.cli._signal_restart")
    def test_rollback_checks_out_tag_and_rebuilds(
        self, mock_restart: Any, mock_run: Any
    ) -> None:
        """sam rollback <tag> must git checkout tag, rebuild, and restart."""
        ...
        assert any("git" in c and "checkout" in c and "v1.0.0" in c for c in calls)
        assert any("docker" in c and "compose" in c and "build" in c for c in calls)
